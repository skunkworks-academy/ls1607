# Learner Assignments

This directory contains practical assignments linked to Luca Sprunt's Individual Development Roadmap. Each assignment must have a public learner-facing specification, a GitHub tracking issue, working templates, an evidence register and a mentor review gate.

## Active assignments

| Assignment | Type | Assignee | Status | Tracking | Workspace |
|---|---|---|---|---|---|
| OWASP Top 10 2025 Content Sprint | Portfolio project | [@Luca-Sprunt](https://github.com/Luca-Sprunt) | Assigned | [Issue #13](https://github.com/skunkworks-academy/ls1607/issues/13) | [`owasp-top-10-2025-content-sprint/`](owasp-top-10-2025-content-sprint/) |
| GitHub Achievements Academy | Foundation task | [@Luca-Sprunt](https://github.com/Luca-Sprunt) | Assigned | [Issue #15](https://github.com/skunkworks-academy/ls1607/issues/15) | [`github-achievements-academy/`](github-achievements-academy/) |

## Standard workflow

1. Read the learner-facing assignment specification.
2. Confirm the task and deadline with the mentor.
3. Create an assignment branch.
4. Complete the provided deliverable templates.
5. Record evidence and sources.
6. Open a pull request linked to the tracking issue.
7. Resolve mentor feedback before merge.
